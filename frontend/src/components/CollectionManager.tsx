...
export function CollectionManager({ isOpen, onClose }: CollectionManagerProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
      ...
      <div
        ref={modalRef}
        className="relative z-10 w-full sm:max-w-md sm:mx-4 bg-background border border-border rounded-t-2xl sm:rounded-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:fade-in duration-200"
      >
        ...
        {/* Collection list */}
        <div className="max-h-[60vh] sm:max-h-80 overflow-y-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
          {isLoading ? (
            ...
            collections.map((collection) => (
              <div
                key={collection.id}
                className="flex items-center gap-3 px-4 py-3 border-b border-border last:border-b-0 hover:bg-muted/50 active:bg-muted/50 transition-colors"
              >
                {editingId === collection.id ? (
                  <div className="flex-1 flex flex-col sm:flex-row gap-2">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <Button size="sm" onClick={handleSaveEdit} disabled={!editName.trim()} className="flex-1 sm:flex-none touch-manipulation">
                        {t.collections.save}
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => setEditingId(null)} className="flex-1 sm:flex-none touch-manipulation">
                  ...
                  <>
                    ...
                    <div className="flex gap-1 shrink-0">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-9 w-9 sm:h-7 sm:w-7 p-0 hover:text-destructive touch-manipulation"
                        onClick={() => handleDelete(collection.id)}
                        disabled={deletingId === collection.id}
                        title={t.collections.delete}
                      >
                        {deletingId === collection.id ? (
                          <svg className="animate-spin h-4 w-4 sm:h-3.5 sm:w-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                          </svg>
                        ) : (
